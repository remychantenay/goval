...
func (v CountryCodeValidator) Validate(val interface{}) (bool, error) {
	...
	if v.ExcludeEu {
		...
		if !countryCodeRegEx.MatchString(str) {return false, fmt.Errorf("invalid country code")}

		b, err := valueExcluded(str, v.Exclude, countryCodeRegEx)
		if !b { return b, err }
	} else {
		...
		if !countryCodeRegExNonEu.MatchString(str) {return false, fmt.Errorf("invalid country code")}

		b, err := valueExcluded(str, v.Exclude, countryCodeRegExNonEu)
		if !b { return b, err }
	}
